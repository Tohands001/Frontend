    ...
    <div className="space-y-8">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500">Global administration and system controls.</p>
        </div>
        {isAdmin && (
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-indigo-600 text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-indigo-700 transition flex items-center shadow-lg shadow-indigo-600/20"
          >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xl p-4 overflow-y-auto">
          <div className="bg-[#1e293b] w-full max-w-2xl rounded-[2.5rem] shadow-2xl p-10 my-8 border border-slate-700/50 animate-in zoom-in-95 duration-200">
            <header className="flex justify-between items-center mb-10 text-white">
              <h3 className="text-3xl font-black flex items-center tracking-tight">
                <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white mr-4 shadow-xl shadow-indigo-600/20">
                  <Icons.UserControl className="w-7 h-7" />
                </div>
                Configure Access
              </h3>
            ...
            <div className="flex space-x-4 mt-12 bg-slate-800/30 p-2 rounded-[2rem] border border-slate-700/50">
              <button
                onClick={() => setIsModalOpen(false)}
                className="flex-1 px-8 py-5 rounded-[1.8rem] font-black text-slate-400 hover:text-white transition-colors uppercase text-xs tracking-widest"
              >
                Discard Changes
              </button>
              <button
                ...
                className="flex-1 px-8 py-5 bg-indigo-600 rounded-[1.8rem] font-black text-white hover:bg-indigo-500 transition-all shadow-2xl shadow-indigo-600/30 disabled:opacity-50 disabled:shadow-none uppercase text-xs tracking-widest"
              >
                Provision Account
              </button>
            </div>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-xl p-4 overflow-y-auto">
          <div className="bg-[#1e293b] w-full max-w-2xl rounded-[2.5rem] shadow-2xl p-10 my-8 border border-slate-700/50 animate-in zoom-in-95 duration-200">
            <h3 className="text-3xl font-black text-white mb-8 flex items-center tracking-tight">
              <div className="w-12 h-12 bg-slate-700 rounded-2xl flex items-center justify-center text-white mr-4 shadow-xl">
                <Icons.UserControl className="w-7 h-7" />
              </div>
              Edit Profile Access
            </h3>
            ...
            <div className="flex space-x-4 mt-12 bg-slate-800/30 p-2 rounded-[2rem] border border-slate-700/50">
              <button
                ...
                className="flex-1 px-8 py-5 rounded-[1.8rem] text-slate-400 font-black uppercase text-xs tracking-widest hover:text-white transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleUpdate}
                className="flex-1 px-8 py-5 bg-indigo-600 rounded-[1.8rem] font-black text-white hover:bg-indigo-500 shadow-xl transition-all"
              >
                Commit Changes
              </button>
            </div>
      ...
      {confirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-10 animate-in fade-in zoom-in duration-200 text-center border border-slate-100">
            <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-red-500/10 ${confirmModal.type === 'REVOKE' ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-600'}`}>
              <Icons.UserControl className="w-10 h-10" />
            </div>
            ...
            <div className="flex flex-col space-y-3">
              <button
                ...
                className={`w-full py-4 text-white rounded-2xl font-black transition-all shadow-xl uppercase text-xs tracking-widest ${confirmModal.type === 'REVOKE' ? 'bg-red-600 hover:bg-red-700 shadow-red-600/20' : 'bg-slate-900 hover:bg-black shadow-slate-900/20'}`}
              >
                Yes, Proceed
              </button>
              <button
                onClick={() => setConfirmModal(null)}
                className="w-full py-4 bg-white border border-slate-200 text-slate-400 rounded-2xl font-black hover:bg-slate-50 transition-all uppercase text-xs tracking-widest"
              >
                Cancel Action
              </button>
            </div>
