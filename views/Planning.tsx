...
const Planning: React.FC<{ store: any; navigateTo: any }> = ({ store, navigateTo }) => {
  const { projects, createProject, currentUser } = store;
  ...
  return (
    <div className="space-y-8">
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold text-slate-900">Project Planning</h2>
          <p className="text-slate-500">Design workflows and configure production stages.</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="bg-indigo-600 text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-indigo-700 transition flex items-center"
        >
        ...
        {accessibleProjects.map((project: Project) => (
          <div key={project.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden group hover:shadow-md transition-all">
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100 flex space-x-2">
              ...
              <button
                className="p-2 border border-slate-200 text-red-500 rounded-lg hover:bg-red-50 transition-colors"
